...
- 2026-03-07: Când utilizatorul cere refactor definitiv pe o pagină mare (ex. Meta card extraction), verific explicit prin căutări țintite că pagina container nu mai are niciun state/type/handler legacy și păstrez tot flow-ul într-un component unic.
- 2026-03-07: Pentru a preveni recurența dublurilor de state în pagini mari frontend, extrag fiecare integrare (ex. Meta) într-un component dedicat cu state locale și las pagina container doar cu layout + compunere.
- 2026-03-07: Pentru rolling Meta, extind punctual scheduler-ul existent (nu unul nou), păstrez eligibilitatea curentă și rulez worker-ul prin `meta_ads_service.sync_client` per chunk/grain ca să evit duplicarea logicii și regresiile Google.
