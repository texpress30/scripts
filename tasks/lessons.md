...
- 2026-02-27: Pentru smoke tests E2E de ingestie, expune sync orchestration filtrabil per client (`client_id`) și include în response un count DB post-insert per customer; astfel verifici rapid că datele chiar au fost persistate, nu doar procesate în memorie.
- 2026-02-27: Pentru incidente cu `inserted_rows_total=0`, include întotdeauna în răspunsul sync trace-ul complet per customer (`gaql_rows_fetched`, `inserted_rows`, `db_rows_last_30_for_customer`, `reason_if_zero`) și rulează fallback GAQL pe resursă alternativă înainte să concluzionezi „fără date”.
