...
- 2026-03-19: Când corectezi lock contention (deadlocks) pe DDL la runtime, NU bypassa pur și simplu rularea acestora în producție (`_ensure_schema`), deoarece aplicația se va baza pe coloane/tabele lipsă și va returna erori 500 (psycopg UndefinedColumn). Folosește mecanismul `pg_advisory_xact_lock` pe un alias de serviciu la nivel de tranzacție pentru a permite schema evolution sigur pe Railway, fără a permite deadlocks între worker-i cron concurenți.
