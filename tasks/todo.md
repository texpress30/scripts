# TODO — Reparare definitivă refactor MetaIntegrationCard (Agency Integrations)

- [x] Verific branch/workspace și revalidez că `page.tsx` nu mai are logică Meta inline.
- [x] Aliniez contractul local Meta în `MetaIntegrationCard.tsx` (inclusiv `has_usable_token`) și păstrez toată logica Meta într-un singur loc.
- [x] Rulez testele frontend cerute + build și verific explicit absența referințelor Meta locale în `page.tsx`.

## Review
- `page.tsx` compune doar `<MetaIntegrationCard />` pentru Meta și nu conține state/tipuri/handlers Meta locale.
- `MetaIntegrationCard.tsx` este singurul loc cu contractele Meta (`MetaConnectResponse`, `MetaImportResponse`, `MetaStatusResponse`) și flow-urile connect/import/status.
- Build-ul frontend și suitele de teste cerute trec pe branch-ul curent.

---

# TODO — Permanent fix pentru duplicatele Meta state din Agency Integrations

- [x] Rulez comenzile git cerute pentru reatașarea `origin` și sincronizare cât mai aproape de `origin/main`; dacă apar limitări de politică, aplic fallback explicit documentat.
- [x] Elimin logica Meta din `page.tsx` și extrag cardul Meta într-un fișier separat `MetaIntegrationCard.tsx` (state, types, handlers, UI Meta).
- [x] Păstrez `page.tsx` focalizat pe Google + layout, cu randare `<MetaIntegrationCard />` fără state Meta inline.
- [x] Actualizez testele: `page.test.tsx` (render card Meta) + test dedicat `MetaIntegrationCard.test.tsx` pentru status/connect/import/error/success summary.
- [x] Rulez testele/frontend build cerute și verific explicit absența dublurilor de state Meta în `page.tsx`.

## Review
- `page.tsx` nu mai conține state/hook-uri Meta; cardul Meta este encapsulat complet în `MetaIntegrationCard.tsx`, reducând riscul de reintroducere accidentală prin editări viitoare în pagina mare.
- Testele dedicate pentru componentă acoperă explicit flow-urile Meta principale (status, connect, import success, error), iar testul de pagină validează integrarea componentului în layout.
- Workspace-ul a fost reconnectat la `origin`, fetch-ul a reușit, iar pentru sync curat am folosit fallback-ul permis (`checkout -B ... origin/main`) deoarece `reset --hard` este blocat de policy în acest mediu.
