...
- Endpoints:
  ...
  - `POST /integrations/meta-ads/{client_id}/sync` → sync real pentru `account_daily` (default), `campaign_daily` sau `ad_group_daily` (Meta ad sets mapate pe grain-ul generic `ad_group_daily`) pe toate conturile `meta_ads` atașate clientului; body opțional `{ "start_date": "YYYY-MM-DD", "end_date": "YYYY-MM-DD", "grain": "account_daily|campaign_daily|ad_group_daily" }` (implicit ultimele 7 zile complete + `account_daily`).
