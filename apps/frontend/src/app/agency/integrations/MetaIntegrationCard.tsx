...
export function MetaIntegrationCard() {
  ...
  const metaHasUsableToken = useMemo(() => {
    if (typeof metaStatus?.has_usable_token === "boolean") return metaStatus.has_usable_token;
    const tokenSource = String(metaStatus?.token_source || "missing").trim().toLowerCase();
    return metaStatus?.status === "connected" || ["database", "env_fallback", "runtime"].includes(tokenSource);
  }, [metaStatus]);
  ...
  return (
    <article className="wm-card p-4">
      ...
      {!metaLoading && !metaHasUsableToken ? <p className="mt-2 text-xs text-amber-700">Importul conturilor Meta necesită un token activ.</p> : null}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        >
          {metaBusy === "connect" ? "Connecting..." : "Connect Meta"}
        </button>
        <button
