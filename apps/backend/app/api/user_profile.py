...
from app.api.dependencies import enforce_agency_navigation_access, get_current_user
...
def get_profile(user: AuthUser = Depends(get_current_user)) -> UserProfileResponse:
    enforce_agency_navigation_access(user=user, permission_key="settings_profile")
