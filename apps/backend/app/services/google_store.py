...
class GoogleSnapshotStore:
    ...
    def _connect(self):
        ...
        if psycopg is None:
            raise RuntimeError("psycopg is required for Google Postgres persistence")
    ...
    def upsert_snapshot(self, *, payload: dict[str, float | int | str]) -> None:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO google_sync_snapshots (
    ...
    def clear(self) -> None:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM google_sync_snapshots")
            conn.commit()


google_snapshot_store = GoogleSnapshotStore()
