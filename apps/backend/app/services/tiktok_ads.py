...
class TikTokAdsService:
    ...
    def run_diagnostics(self) -> dict[str, object]:
        ...
        token_expires_at = self._token_expires_at()
        has_usable_token = token != ""
        api_version = str(settings.tiktok_api_version or "").strip()

        oauth_ok = False
        advertiser_accounts_count = 0
        accessible_accounts: list[dict[str, object]] = []
        last_error: str | None = None
        warnings: list[str] = []

        if not oauth_configured:
            warnings.append("TikTok OAuth is not configured. Set TIKTOK_APP_ID, TIKTOK_APP_SECRET, and TIKTOK_REDIRECT_URI.")

        if has_usable_token:
            try:
                accessible_accounts = self.list_accessible_advertiser_accounts()
                advertiser_accounts_count = len(accessible_accounts)
                oauth_ok = True
            except Exception as exc:  # noqa: BLE001
                last_error = str(exc)
                warnings.append(f"Failed to list advertiser accounts: {str(exc)[:200]}")
        else:
        ...
        return {
            ...
            "token_expires_at": token_expires_at,
            "has_usable_token": has_usable_token,
            "advertiser_accounts_count": advertiser_accounts_count,
            "sample_advertiser_accounts": [
                {"id": str(a.get("advertiser_id", a.get("id", ""))), "name": str(a.get("advertiser_name", a.get("name", "")))}
                for a in accessible_accounts[:10]
            ],
