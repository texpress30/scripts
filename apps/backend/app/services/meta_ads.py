...
class MetaAdsService:
    ...
    def _fetch_ad_group_daily_insights(
        self,
        *,
        account_id: str,
        start_date: date,
            ...
            with self._memory_lock:
                for row in rows:
                    ...
                    replace_idx = next(
                        (
                            idx
                            for idx, existing in enumerate(self._memory_ad_group_daily_rows)
                            if (
                                str(existing.get("platform") or ""),
                                str(existing.get("account_id") or ""),
                                str(existing.get("ad_group_id") or ""),
                                str(existing.get("report_date") or ""),
                            )
                            == key
                        ),
                    ...
                    if replace_idx is None:
                        self._memory_ad_group_daily_rows.append(dict(row))
                    else:
                        self._memory_ad_group_daily_rows[replace_idx] = dict(row)
            return len(rows)
