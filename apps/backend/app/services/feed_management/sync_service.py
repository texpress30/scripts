        ...
        if source.credentials_secret_id:
            try:
                ...
                for key in ("api_key", "api_secret_key", "access_token"):
                    secret = integration_secrets_store.get_secret(provider="shopify", secret_key=key, scope=source.credentials_secret_id)
                    if secret:
...
class FeedSyncService:
    ...
    async def run_sync(self, feed_source_id: str) -> FeedImportResponse:
        ...
        try:
            async for product in connector.fetch_products():
                ...
                if total_count % SYNC_BATCH_SIZE == 0:
                    feed_import = self._import_repo.update_status(
                        feed_import.id, status=FeedImportStatus.in_progress,
                        total_products=total_count, imported_products=imported_count, errors=errors,
                    )

            feed_import = self._import_repo.update_status(
                feed_import.id, status=FeedImportStatus.completed,
                total_products=total_count, imported_products=imported_count, errors=errors,
            )
        except Exception as exc:
            ...
            feed_import = self._import_repo.update_status(
                feed_import.id, status=FeedImportStatus.failed,
                total_products=total_count, imported_products=imported_count, errors=errors,
            )
    ...
    async def run_sync_background(self, feed_source_id: str) -> None:
        try:
            ...
            logger.info("Background sync completed for source %s: status=%s imported=%d/%d",
                        feed_source_id, result.status, result.imported_products, result.total_products)
        except Exception:
