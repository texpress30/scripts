...
class SnapchatSnapshotStore:
    ...
    def _connect(self):
        ...
        if psycopg is None:
            raise RuntimeError("psycopg is required for Snapchat Postgres persistence")
    ...
    def upsert_snapshot(self, *, payload: dict[str, float | int | str]) -> None:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO snapchat_sync_snapshots (
    ...
    def clear(self) -> None:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM snapchat_sync_snapshots")
            conn.commit()


snapchat_snapshot_store = SnapchatSnapshotStore()
